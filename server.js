          ...
          messages: [
            {
              ...
              content: [
                ...
                {
                  type: 'text',
                  text: `Extract ALL info from this business card as JSON only (no markdown):
{"brandName":"","personName":"","designation":"","department":"","email":"","phone":"","alternatePhone":"","website":"","address":"","city":"","state":"","country":"","pincode":""}`,
      ...
      cardData = JSON.parse(
        claudeRes.data.content[0].text.trim().replace(/```json|```/g, '').trim()
      );
    ...
    // ── Extract only: return for review, skip saving ──────────────────────
    if (extractOnly) {
